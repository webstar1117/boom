...
@section('title') Lives @endsection
@section('css')
    <!-- DataTables -->
                    ...
                    <table id="datatable" class="table table-bordered dt-responsive nowrap"
                        ...
                        <tbody>
                            @foreach ($lives as $key => $life)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $life->memorial->site_address }}</td>
                                    <td>{{ $life->user->firstname }}&nbsp;{{ $life->user->lastname }} </td>
                                    <td>{{ $life->title }}</td>
                                    <td>{{ strlen( $life->content) > 20 ? substr( $life->content, 0, 20) :  $life->content }}</td>
                                    <td>
                                        <img class="w-50" src="{{asset('/assets/media/'.$life->media->name)}}" />
                                    </td>
                                    <td style="cursor:pointer"><i class="fas fa-window-close text-danger"
                                            onclick="ReadyDel('{{ $life->id }}')"></i></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="modal-dialog modal-lg ">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="edit_form" method="get" action="{{ url('/admin/lives/delete') }}">
