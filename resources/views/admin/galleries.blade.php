                    ...
                    <table id="datatable" class="table table-bordered dt-responsive nowrap"
                        ...
                        <tbody>
                            @foreach ($galleries as $key => $gallery)
                                <tr>
                                    <td>{{ $key + 1 }}</td>
                                    <td>{{ $gallery->memorial->site_address }}</td>
                                    <td>{{ $gallery->user->firstname }}&nbsp;{{ $gallery->user->lastname }} </td>
                                    <td>
                                        @if($gallery->media->type=='video')
                                        <video class="w-50 height-inherit" controls>
                                            <source
                                                src="{{ asset('/assets/media/' . $gallery->media->name) }}"
                                                type="{{ $gallery->media->type }}/{{ $gallery->media->extension }}">
                                        </video>
                                        @elseif($gallery->media->type=='audio')
                                        <audio class="w-50" controls style="margin-top:43px">
                                            <source
                                                src="{{ asset('/assets/media/' . $gallery->media->name) }}"
                                                type="{{ $gallery->media->type }}/{{ $gallery->media->extension }}">
                                        </audio>
                                        @elseif($gallery->media->type=='image')
                                        <img class="w-50" src="{{asset('/assets/media/'.$gallery->media->name)}}" />
                                        @endif
                                    </td>
                                    <td style="cursor:pointer"><i class="fas fa-window-close text-danger"
                                            onclick="ReadyDel('{{ $gallery->id }}')"></i></td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
        ...
        <div class="modal-dialog modal-lg ">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="edit_form" method="get" action="{{ url('/admin/galleries/delete') }}">
